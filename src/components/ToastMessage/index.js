...
const ToastMessage = () => {
  ...
  }, [state, dispatch, closeAlert])

  return (
    <div
      className={`position-fixed right-0 fs-18px z-index-max nav-gap-2vh ${Style.alert_box_wrapper}`}
    >
      {state.toast_message.message && (
        <div
          className={`alert ${
            state.toast_message.type === "success"
              ? `alert-success`
              : `alert-danger`
        ...
        >
          <button
            type="button"
            className="btn-close position-static pt-2"
            aria-label="Close"
            onClick={closeAlert}
          >
